# uptime-kuma-agent — Concept Overview
...
```json
{
  "mcpServers": {
    "uptime-kuma-agent": {
      "command": "uv",
      "args": ["run", "--with", "uptime-kuma-agent", "uptime-mcp"],
      "env": {}
    }
  }
}
...
```bash
uptime-mcp --transport streamable-http --port 8001
```
